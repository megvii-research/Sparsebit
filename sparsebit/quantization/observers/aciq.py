...
class Observer(BaseObserver):
    ...
    def calc_minmax(self):
        batch_size = torch.cat(self._data_cache, axis=0).shape[0] if self.qdesc.ch_axis > 0 else 1
        data = self.get_calibration_data(c_first=True)

        laplace_min_val, laplace_max_val = self.calc_laplace_minmax(data)
        scale_laplace, _ = self.calc_qparams_with_minmax(laplace_min_val, laplace_max_val)
        mse_laplace = mse_loss((torch.clamp(data, laplace_min_val, laplace_max_val)/scale_laplace).round()*scale_laplace, data)
        ...
        mse_gaus = mse_loss((torch.clamp(data, gaus_min_val, gaus_max_val)/scale_gaus).round()*scale_gaus, data)
        ...
        mse_minmax = mse_loss((torch.clamp(data, naive_min_val, naive_max_val)/scale_minmax).round()*scale_minmax, data)

        self.min_val = torch.where(mse_gaus < mse_laplace, gaus_min_val, laplace_min_val)
        self.min_val = torch.where(mse_minmax < mse_gaus, naive_min_val, self.min_val).to(self.device)
        self.max_val = torch.where(mse_gaus < mse_laplace, gaus_max_val, laplace_max_val)
        self.max_val = torch.where(mse_minmax < mse_gaus, naive_max_val, self.max_val).to(self.device)
