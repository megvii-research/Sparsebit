...
class Observer(nn.Module):
    ...
    def calc_qparams(self, bit_allocation):
        if bit_allocation:
            assert len(self.cfg.OBSERVER.BIT_CHOICES)>0, "Please assign bit choices before applying bit allocation!"
        ...
        scale, zero_point = self.calc_qparams_with_minmax(min_val, max_val, self.qdesc.bit)
