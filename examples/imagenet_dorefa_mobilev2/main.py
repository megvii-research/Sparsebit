...
def main():
    ...
    if args.multiprocessing_distributed:
        ...
        if args.gpu is None:
            if not torch.cuda.is_available():
                raise "no cuda device exist, we assume you have at least one cuda device"
            else:
                args.gpu = 0  # default set gpu device 0
    ...
    train_dataset = imagenet_dataset(train_transforms, mode="train")
    val_dataset = imagenet_dataset(val_transforms, mode="validation")
    if args.multiprocessing_distributed:
    ...
    model.model.features_0_0.weight_quantizer.set_bit(bit=8)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, train_loader_len)
        #scheduler.step()
...
def train(train_loader, model, criterion, optimizer, epoch, args, train_loader_len):
