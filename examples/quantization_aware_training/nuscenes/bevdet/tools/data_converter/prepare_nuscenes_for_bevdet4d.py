...
def add_adj_info():
    ...
    for set in ["train", "val"]:
        ...
        for id in range(len(dataset["infos"])):
            ...
            for adj in ["next", "prev"]:
                ...
                for cam in [
                    ...
                    while count < max_adj:
                        ...
                        adj_list[cam].append(
                            dict(
                                data_path="./data/nuscenes/" + sd_adj["filename"],
                        ...
                        count += 1
                for count in range(
                    interval - 1, min(max_adj, len(adj_list["CAM_FRONT"])), interval
                ):
                    ...
                    cam_infos = dict(
                        CAM_FRONT=dict(
                            data_path=adj_list["CAM_FRONT"][count]["data_path"]
                        )
                    )
                    for cam in [
                        "CAM_FRONT",
            ...
            posi_pre = np.array(
                dataset["infos"][previous_id]["ego2global_translation"],
                dtype=np.float32,
            )
            posi_next = np.array(
            ...
            dataset["infos"][id]["gt_velocity"] = dataset["infos"][id][
                "gt_velocity"
            ] - velocity_lidar.reshape(1, 2)

        with open(
            "./data/nuscenes/nuscenes_infos_%s_4d_interval%d_max%d.pkl"
            % (set, interval, max_adj),
            "wb",
        ) as fid:
