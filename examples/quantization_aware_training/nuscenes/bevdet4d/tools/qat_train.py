...
def parse_args():
    ...
    parser.add_argument("qconfig", help="train config file path")
    parser.add_argument("pretrained", help="train config file path")
...
def main():
    ...
    from projects.mmdet3d_plugin.models.detectors import BEVDet4dTraced, BEVDet4dForward

    # build qmodel
    checkpoint = load_checkpoint(model, args.pretrained, map_location="cpu")
    model = BEVDet4dTraced(model, cfg.data.samples_per_gpu).cuda()

    qconfig = parse_qconfig(args.qconfig)
    with torch.no_grad():
        qmodel = QuantModel(model, config=qconfig)
    # build calibration dataloader
    calib_dataset = build_dataset(cfg.data.train, dict(test_mode=True))
    calib_dataloader = build_dataloader(
        calib_dataset,
    ...
    with torch.no_grad():
        for i, data in enumerate(calib_dataloader):
            img_inputs = [v.cuda() for v in data["img_inputs"]]
            result = qmodel(img_inputs)
            cur_size += cfg.data.samples_per_gpu
    ...
    running_qmodel = BEVDet4dForward(model, qmodel)
