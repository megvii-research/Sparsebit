...
def parse_args():
    ...
    parser.add_argument("qconfig", help="test config file path")
...
def main():
    ...
    model = BEVDet4dTraced(model, cfg.data.samples_per_gpu)
    ...
    calib_size, cur_size = 1, 0
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            img_inputs = [v.cuda() for v in data["img_inputs"][0]]
    ...
    if "CLASSES" in checkpoint.get("meta", {}):
        running_qmodel.CLASSES = checkpoint["meta"]["CLASSES"]
    else:
        running_qmodel.CLASSES = dataset.CLASSES
    # palette for visualization in segmentation tasks
    if "PALETTE" in checkpoint.get("meta", {}):
        running_qmodel.PALETTE = checkpoint["meta"]["PALETTE"]
